...
def load_configuration():
    ...
    try:
        ...
        raise json.JSONDecodeError(
            f"Invalid JSON in configuration file {config_path}: {e.msg}", e.doc, e.pos
        )
...
def is_arcgis_error(arcgis_server_response):
    ...
    """
    return arcgis_server_response is None or "error" in arcgis_server_response
...
def cache_response(arcgis_server_response: dict) -> dict | None:
    ...
    if not is_arcgis_error(arcgis_server_response):
        ...
        arcgis_server_response["expiresDate"] = int(
            arcgis_server_response.get("expires_in", 0)
        ...
        try:
            ...
            logger.error(
                f"Cannot write cache file at '{configuration.get('cacheFile', 'token_cache.json')}': {error}"
            )
    ...
    try:
        ...
        if is_token_expired(cached_token):
            expires_date = datetime.fromtimestamp(
                cached_token.get("expiresDate", 0) / 1000
            )
    ...
    try:
        if not force_refresh:
            ...
            try:
                ...
                logger.info(
                    f"Cache unavailable ({cache_error}), requesting new token from server"
                )
